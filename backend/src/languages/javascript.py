...
3. **[중요: 원본 코드 미포함]**: 사용자가 입력한 소스 코드는 이미 존재한다고 가정합니다. 출력물에 입력받은 변수, 함수, 클래스를 다시 정의하지 마십시오.
4. **[Import 가이드]**: 코드 상단에 반드시 원본 파일의 위치에 맞게 경로를 수정하라는 안내 주석과 플레이스홀더를 포함하십시오.
   ```javascript
   // [TODO] 원본 파일 위치에 맞게 경로를 수정하세요.
   // import { {target} } from './source_file';
   ```
5. **모듈 시스템 일관성 유지**:
...
6. 외부 의존성이 있다면 `jest.mock()`을 사용하여 모킹합니다.
7. 정상 동작, 엣지 케이스, 예외 처리를 모두 검증하는 테스트를 포함합니다.
8. 모든 설명과 주석은 한국어로 작성합니다.
