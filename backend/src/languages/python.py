...
3. **[중요: 원본 코드 미포함]**: 사용자가 입력한 소스 코드는 이미 존재한다고 가정합니다. 출력물에 입력받은 함수나 클래스를 다시 정의하지 마십시오.
4. **[Import 가이드]**: 코드 상단에 반드시 다음과 같은 형식의 주석과 플레이스홀더를 포함하십시오.
   ```python
   # [TODO] 테스트 대상 함수를 import 하세요.
   # from your_module import {target_function}
   ```
